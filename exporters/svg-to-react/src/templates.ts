...
export function applicableComponentTemplate(svg: string, name: string): string {
  if (exportConfiguration.customComponentTemplate) {
    return sevComponentTemplate(svg, name);
  } else {
    return exportConfiguration.typescript
      ? componentTSTemplate(svg, name)
      : componentJSTemplate(svg, name);
  }
}
...
export function componentCustomTemplate(
  ...
  if (!template.includes('{{svg}}'))
    throw new Error('Template must contain {{svg}} tag');
...
export function indexCustomTemplate(
  template: string,
  path: string,
  name: string
): string {
  if (!template.includes('{{path}}'))
    throw new Error('Template must contain {{path}} tag');
  if (!template.includes('{{name}}'))
    throw new Error('Template must contain {{name}} tag');
